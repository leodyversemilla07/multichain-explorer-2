...
def file_dir_name(file_name):
    return (
        os.path.dirname(
            os.path.realpath(str(file_name).replace("~", os.path.expanduser("~")))
        ).rstrip("/")
        + "/"
    )
...
def become_daemon():
    # Fork is Unix-only, not available on Windows
    if not hasattr(os, "fork"):
        print_error("become_daemon() is not supported on Windows")
        return
